function QuantPitchContours_leila(syl,NFFT,overlap,sigma)
% QuantPitchContours
...
for i = 1:size(notmats,1)
    ...
    cbin_fn = notmats(i, 1:notmat_strid - 1);

    
    if strcmp(cbin_fn(end-3:end),'cbin')
        [rawsong, fs] = evsoundin('.', cbin_fn,'obs0'); %load song from current .cbin file
    elseif strcmp(cbin_fn(end-3:end),'.wav') %if file is .wav
        [rawsong,fs] = audioread(cbin_fn);
        rawsong = resample(rawsong,30000,fs);
        fs = 30000;
    elseif strcmp(cbin_fn(end-3:end),'.rhd')
        matname = [cbin_fn(1:end-3),'mat'];
        load(matname)
        rawsong = board_adc_data;
        fs = frequency_parameters.amplifier_sample_rate;
    elseif strcmp(cbin_fn(end-3:end),'.mat')
        matname = [cbin_fn(1:end-3),'mat'];
        load(matname)
        rawsong = board_adc_data;
        fs = frequency_parameters.amplifier_sample_rate;
...
%                 if quantify_mini_mic;mini_mic_song = resample(mini_mic_song,p,q);end
    ...
    if ~exist('pitchContours','var')
        pitchContours(lblsize) = struct('contour',[],...
            'timeVector',[],...
            'NFFT',[],...
            'overlap',[],...
            'sigma',[]);
    ...
    for j = 1:length(ids)
        ...
        [~,freqsVector,timeVector,powerVector] = spectrogram(sylFromRawsong,window,overlap,NFFT,fs);
            ...
            if strcmp(err.identifier,'signal:welchparse:invalidSegmentLength')
                disp(['Rendition ' num2str(ids) ' could not be converted to spectrogram'])
