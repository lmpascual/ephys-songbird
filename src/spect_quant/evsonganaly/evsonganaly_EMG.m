...
function varargout = evsonganaly_EMG(varargin)
...
% Last Modified by GUIDE v2.5 22-Aug-2012 14:04:08
...
[INPUTFILES,ChanSpec,EMGChanSpec]=evloadfile_EMG;
...
handles.EMGChanSpec=EMGChanSpec;
handles.NFILE=1;
handles.SPECTH=0.01;
...
elseif strfind(pwd,'g43g47')
    ...
    handles.SEGTH=200;
    handles.SM_WIN=2.0;
elseif strfind(pwd,'bl5wh1')
    handles.MINDUR=20.0;%in msec
    handles.MININT=2.0;%in msec
    handles.SEGTH=50;
    handles.SM_WIN=2.0;
elseif strfind(pwd,'gr42rd52')
    handles.MINDUR=20.0;%in msec
    handles.MININT=2.0;%in msec
    handles.SEGTH=50;
    handles.SM_WIN=2.0;
elseif strfind(pwd,'lb10yw25')
    handles.MINDUR=20.0;%in msec
    handles.MININT=2.0;%in msec
    handles.SEGTH=50;
    handles.SM_WIN=2.0;
elseif strfind(pwd,'lb59wh59')
    handles.MINDUR=20.0;%in msec
    handles.MININT=12.0;%in msec
    handles.SEGTH=25;
    handles.SM_WIN=2.0;
elseif strfind(pwd,'gr49rd59')
    handles.MINDUR=20.0;%in msec
    handles.MININT=2.0;%in msec
    handles.SEGTH=1500;
    handles.SM_WIN=2.0;
...
%link the x axes
linkaxes([handles.SpecGramAxes,handles.LabelAxes,handles.SmoothAxes,handles.NEWAXIS],'x');
...
PlotDataFile_EMG(hObject,handles);
...
switch notmat_exists
    ...
    case 2 % exist returns 2 for argument 'file' when the file exists
        ...
        % because save command is not smart enough to take a list of vars)
        return
end
...
%spectitle=handles.INPUTFILES(handles.NFILE).fname;ch
